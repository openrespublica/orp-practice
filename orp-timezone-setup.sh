#!/bin/bash
# orp-timezone-setup.sh — Alpine System Timezone Configuration
set -eu
...
if [ ! -f "$ZONEINFO_FILE" ]; then
    echo "[!] Zoneinfo file not found: $ZONEINFO_FILE"
    exit 1
fi
...
if [ "$CURRENT_TZ_FILE" = "$TARGET_TZ" ]; then
    ...
    echo "[✔] /etc/timezone updated."
fi

# ── 4. Shell environment ────────────────────────────────────────────
PROFILE_FILE="$HOME/.profile"
if grep -q 'export TZ=' "$PROFILE_FILE" 2>/dev/null; then
    echo "[✔] TZ already set in $PROFILE_FILE"
else
...
if date | grep -qE '(PHT|PST|\+0800)'; then
    ...
    echo "[!] UTC offset does not match +0800. Verify manually."
fi
