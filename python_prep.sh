...
VENV_DIR="$SCRIPT_DIR/.venv"                                                              REQ_IN="$SCRIPT_DIR/requirements.in"
REQ_FILE="$SCRIPT_DIR/requirements.txt"                                                   CA_CERT="/etc/ssl/certs/ca-certificates.crt"
PIP_LOG="$SCRIPT_DIR/pip-secure.log"

echo "[*] Initializing Python Environment Setup"

# ── 1. System Dependencies (Alpine via apk) ──────────────────────
echo "[*] Ensuring required Alpine system packages are installed..."
# Note: Alpine provides python3 venv capabilities natively in the python3 package
apk add --no-cache python3 py3-pip libmagic ca-certificates
...
if [ -d "$VENV_DIR" ]; then
    echo "[!] Virtual environment already exists at $VENV_DIR"                            else
    echo "[*] Creating virtual environment..."
    python3 -m venv "$VENV_DIR"
fi

# POSIX standard for sourcing
. "$VENV_DIR/bin/activate"

echo "[*] Upgrading core tools..."
python3 -m pip install --quiet --upgrade pip setuptools wheel

# ── 3. Bootstrap Pin Check ───────────────────────────────────────
MISSING_PINS=""
for PKG in pip setuptools wheel; do
    if ! grep -iq "^${PKG}==" "$REQ_IN"; then
        MISSING_PINS="$MISSING_PINS $PKG"
    fi
done
...
if [ -n "$MISSING_PINS" ]; then
    ...
    # POSIX compatible python output extraction
    ...
    mv "$TMP" "$REQ_IN"
    echo "[✔] Pins added to requirements.in."
fi
...
echo "[*] Compiling locked requirements.txt (with hashes)..."
"$VENV_DIR/bin/pip-compile" --generate-hashes --quiet "$REQ_IN" --output-file "$REQ_FILE"
[ -s "$REQ_FILE" ] || { echo "[✘] ERROR: requirements.txt not created" >&2; exit 1; }
...
if python3 -m pip_audit --progress-spinner off 2>/dev/null; then
    echo "[✔] pip-audit passed — no known vulnerabilities."
else
    echo "[!] pip-audit found issues. Check $PIP_LOG for details."
fi

python3 -m pip freeze > "$SCRIPT_DIR/requirements.lock"

echo "[✔] Python Environment Ready."
echo "Venv: $VENV_DIR | Lockfile: requirements.lock"
