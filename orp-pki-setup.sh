#!/bin/sh
# orp-pki-setup.sh — Alpine Minimal Sovereign PKI Setup                                   set -eu
...
echo "[*] Target Directory: $PKI_DIR"                                                                                                                                               if ! command -v openssl >/dev/null 2>&1; then
    echo "[*] Installing openssl..."                                                          apk add --no-cache openssl                                                            fi
...
if [ -f sovereign_root.crt ]; then
    ...
    openssl req -x509 -new -nodes -key sovereign_root.key -sha256 -days 3650 \                    -out sovereign_root.crt \
        -subj "/C=PH/ST=Negros Oriental/L=Sibulan/O=ORP Sovereign/CN=ORP Root CA" 2>/dev/null
    echo "[✔] Root CA generated."
fi                                                                                                                                                                                  # 2. Server Certificate
if [ -f orp_server.crt ]; then
    echo "[!] Server certificate exists — skipping."
else                                                                                          echo "[*] Generating Server Certificate (1 year)..."                                      openssl genrsa -out orp_server.key 2048 2>/dev/null                                       openssl req -new -key orp_server.key -out orp_server.csr \
...
if [ -f operator_01.crt ]; then
    ...
    openssl genrsa -out operator_01.key 2048 2>/dev/null                                      openssl req -new -key operator_01.key -out operator_01.csr \
...
if getent group nginx >/dev/null 2>&1; then
    chgrp nginx "$PKI_DIR"/*.crt "$PKI_DIR"/*.key 2>/dev/null || true                         chmod 640 "$PKI_DIR"/*.key 2>/dev/null || true
fi
                                                                                          # 6. Verification
