# main.py — ORP Engine · PDF Stamp & Anchor Service (IMPROVED v4)
...
def run_git_command(cmd: list, description: str) -> bool:
    """
    Runs a git command using the ephemeral SSH agent from the boot
    script. SSH_AUTH_SOCK and GNUPGHOME are read fresh on every call
    so that Gunicorn worker forks always see the correct live values
    rather than a snapshot taken at import time.
    """
    try:
        git_env = os.environ.copy()

        # ── Re-read agent socket path at call time ────────────────
        # os.environ may be stale in a forked Gunicorn worker.
        # The boot script exports both vars; if they drifted, reading
        # /proc/self/environ would give the original exec values —
        # but relying on the module-level constants (set at startup
        # from the exec'd environment) is correct here because exec
        # replaces the process image, so worker forks inherit the
        # right values.  We still override explicitly to be safe.
        ...
        # ── SSH command ───────────────────────────────────────────
        # BatchMode=yes  — never prompt; fail immediately if the
        #                  agent key is not accepted. This prevents
        #                  the 30-second hang seen when SSH falls
        #                  back to a passphrase-protected disk key.
        # IdentitiesOnly=yes — use ONLY what the agent offers;
        #                      ignore ~/.ssh/id_* files entirely.
        # IdentityAgent   — point explicitly at the gpg-agent SSH
        #                   socket so the subprocess finds it even
        ...
        #   attempt its own GPG commit signing; signing is done by
        #   the shell-level git config set in orp_configure_git().
        #   Forcing it off here avoids a second gpg-agent call that
        #   would use a different GNUPGHOME context.
        defensive_cmd = [
            "git",
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd=REPO_PATH,   # always run inside the repo; removes
                             # the need for -C on every sub-command
            timeout=30,
        )
        ...
        # Distinct log message — previously this showed as a generic
        # CalledProcessError which obscured the real cause.
        logger.error(
...
def _verify_ssh_agent(git_env: dict) -> None:
    """
    Diagnostic helper — logs which keys the SSH agent exposes to
    the subprocess. Called once at the start of sync_to_github so
    the operator can confirm the ephemeral key is visible.
    Failures here are non-fatal; they only affect logging.
    """
    try:
        result = subprocess.run(
            ["ssh-add", "-l"],
...
def sync_to_github(json_path: str, record: dict) -> None:
    """
    Background daemon thread: writes the manifest, commits the new
    record JSON, and pushes to GitHub Pages over SSH.
    Includes self-healing for rebase conflicts.
    """
    with git_lock:
        try:
            ...
            # ── One-time diagnostic: confirm agent key is reachable ──
            # Build a minimal env just for the check.
